...
namespace AsciiArt
{
    public class AsciiArtFont
    {
		#region Private variables
		private const string EmbeddedFilePrefix = "AsciiArt.Fonts.";
		...
		#region Constructors
		/// <summary>
		/// Instantiate from a font file
		/// </summary>
		/// <param name="streamReader">StreamReader for font file</param>
		private AsciiArtFont(StreamReader streamReader)
		{
		...
		public AsciiArtFont(string file)
		...
		/// <summary>
		/// Return a font from it's friendly name
		/// </summary>
		/// <param name="name">Friendly name for the font</param>
		/// <returns>A font corresponding to the friendly name</returns>
		public static AsciiArtFont FontFromName(string name)
		{
			var resourceName = EmbeddedFilePrefix + name + EmbeddedFileExtension;
			if (resourceName == null)
			{
				throw new ArgumentException("FontFromName has invalid name");
			}
			var resourceStream = Assembly.GetExecutingAssembly().GetManifestResourceStream(resourceName);
			Debug.Assert(resourceStream != null, "resourceStream != null");
			var sr = new StreamReader(resourceStream);
			return new AsciiArtFont(sr);
		}
